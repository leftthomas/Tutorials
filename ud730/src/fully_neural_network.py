...
Previously in notmnist, we created a pickle with formatted datasets for training,
development and testing on the notMNIST dataset.

The goal of this assignment is to progressively train deeper and more accurate
...
# The graph will be similar, except that instead of holding all the training data into a
# constant node, we create a Placeholder node which will be fed actual data at every call
# of session.run().
batch_size = 128
...
# with rectified linear units nn.relu() and 1024 hidden nodes. This model should
